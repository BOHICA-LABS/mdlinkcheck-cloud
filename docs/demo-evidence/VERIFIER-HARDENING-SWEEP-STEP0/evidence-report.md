...
| AC-7 | VEF selftest suite: 51/51 pass (each proved clean-pass + defect-fail) | PASS |
...
- `AC-007-vef-selftest.txt` — VEF selftest suite confirming 51/51 pass
...
VEF selftest suite (`scripts/tests/test-vef.py`): 51/51 pass.
Each test proved DEFECT PRESENT (verifier exits non-zero) and DEFECT ABSENT (verifier exits 0).
Covers T01-T49 including B-1 residual (T22-T24), B-3 structural (T25-T26), B2-4 (T30),
...
probes confirming unconditional failure on ADR and EI sides (T47-T48), and M-2 Case-G
positive-pinning probe confirming wrong-figure-hidden-in-stripped-span is detected (T49).
