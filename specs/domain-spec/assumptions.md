...
| ASM-004 | Platform matrix is macOS + Linux + Windows | confirmed (human) | Confirmed | HIGH — missed platform bugs | CI matrix with all three platforms; Windows path-separator handling in path resolution |
| ASM-005 | R8 5-second target is achievable in release build on a modern laptop with warm filesystem cache for 500 `.md` files with offline checks only | unvalidated | Medium | MEDIUM — R8 becomes unmeasurable NFR | Build acceptance corpus of 500 `.md` files; run `hyperfine` in release mode on macOS and Linux CI runners; report p95 over 10 runs. |
