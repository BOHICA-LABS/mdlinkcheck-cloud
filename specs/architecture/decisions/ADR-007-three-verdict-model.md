...
date: 2026-08-05
version: "1.2"
subsystems_affected: [SS-08, SS-09, SS-11, SS-10, SS-14]
supersedes: null
superseded_by: null
---

# ADR-007: Three-Verdict Model — alive / broken / indeterminate
...
Use a three-verdict model with a closed reason-code taxonomy:

- **alive** — an external link definitely resolves (2xx, 3xx with valid redirect target)
- **clean** — a local/internal link definitely resolves (file exists, anchor found;
  `clean` findings are NOT emitted in output)
- **broken** — the link definitely does not resolve; exit code 1 applies
- **indeterminate** — the check could not produce a definitive verdict (transient
  server-side conditions: timeout, 429, 5xx, bot-blocking); does NOT trigger exit 1

All three external-URL verdicts (`alive`, `broken`, `indeterminate`) appear in
JSON output (DI-010). Only `broken` findings contribute to a non-zero exit code.
`indeterminate` findings are surfaced in output but do not trigger a CI failure
...
- `http-error` — 404, 410, or 400 after HEAD+GET fallback
...
- `http-indeterminate` — 429, 5xx, or bot-blocking 403/999 after GET fallback

I/O error (exit 2, not a verdict):
- `target-unreadable` — source `.md` file exists but cannot be read

`alive` and `clean` carry no reason code.
...
**Three-verdict model enables future `--strict-indeterminate` flag:** If users want
`indeterminate` to count as broken (for paranoid CI environments), they can pass
`--strict-indeterminate` (DD-016). The three-verdict model makes this future flag
...
- VP-007 Kani proof verifies the exit-code mapping
...
- DI-010: Anti-false-positive invariant; three-verdict model mandate
